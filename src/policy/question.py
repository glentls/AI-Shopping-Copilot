...
def recommendation_window(state: ConversationState, top_k: int = 10) -> int:
    ...
    turn 10 cannot produce a hit. On the public set no session has EVER hit
    after turn 4, while all 27 misses sit at rank 11-300 -- retrieved, just
    never shown. Paging one window deeper per silent turn is worth +0.079
    TechnicalScore (hit 0.865 -> 0.975).
