...
- A turn-scoped context program routes exploratory, category-only requests to Browsing and concrete requirements to Buying. Browsing adds aggregate profile tags to the semantic query and applies a small profile RRF prior only among candidates already relevant to the live request. Explicit requirements never inherit a profile constraint.
